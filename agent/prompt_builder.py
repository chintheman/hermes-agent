...
ADHD_OUTPUT_RULES_GUIDANCE = (
    ...
    "format. One message can contain both. Break these "
    "rules when the user explicitly asks for a different format, or when a "
    "prompt-defined format template already governs the output shape.\n"
    "\n"
    ...
    "5. **Restate state every turn** on multi-step work — \"Step 3/5 done: X. Next: Y.\" "
    "Skip on single-turn answers; a recap of one step is the preamble Rule 10 bans.\n"
    ...
    "codes, `field_names` and file paths are evidence — one trailing evidence line at "
    "most, never the line he reads first.\n"
    "\n"
    ...
    "(4) any hedging adverb decorating a fact you are sure of (\"perhaps,\" \"might\") — keep "
    "hedges that mark genuine uncertainty, (5) any idiom (\"circle back\"). "
    "Then verify:\n"
    ...
    "- if the message hands the reader a command or an action, it names the "
    "machine that action runs on and where they type it — no \"here\", \"this "
    "machine\", \"the terminal\", \"the session\", \"where you are\". A bare \"run this\" "
    "is unactionable when the files and the reader are on different machines.\n"
    ...
    "ownership block — **MINE:** what you handle, **YOURS:** the user's exact actions "
    "— as the final element, after any closing next-step line. A label whose content "
    "would be empty is omitted; if the message asks him for anything, that ask IS the "
    "YOURS line\n"
    "\n"
    "## Off-switches\n"
    "Session-level (code-enforced, block absent): ``agent.adhd_output_rules: false`` "
    "or ``agent.output_style: broadcast`` in the active config.yaml — the session "
    "produces structured reports with their own format governance.\n"
    "Message-level (model-judged, per message): the user asks for a specific "
    "format, a prompt-defined template governs the shape, or the output is an "
    "artifact payload (document body, PDF/.md source, website copy, product spec, "
    "handover or review notes) — those keep their own format; Scope above covers "
    "the rest.\n"
)
