...
def test_add_contributor_refuses_a_case_collision(tmp_path, monkeypatch):
    ...
    # Assert nothing was WRITTEN, not that the lowercase path is absent. On a
    # case-insensitive filesystem (macOS, Windows) the two names are the same
    # path, so the lowercase one "exists" purely because setup created the
    # uppercase one -- which made this test fail on exactly the platforms the
    # guard exists to protect. Checking the content proves the refusal held on
    # both filesystem kinds: one mapping, still the original owner.
