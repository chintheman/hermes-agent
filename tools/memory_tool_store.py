...
class MemoryStore:
    ...
    def add(self, target: str, content: str) -> Dict[str, Any]:
        ...
        def _add(entries, limit):
            ...
            # Semantic near-duplicates — same lesson reworded (Garmin sync-gap
            # incident, Aug 26 2026). Memory-target only: USER.md facts share
            # identifiers legitimately.
            if target == "memory":
    ...
    def apply_batch(self, target: str, operations: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        def _apply(entries, limit):
            ...
            for i, op in enumerate(ops):
                act = op.get("action")
                content2 = (op.get("content") or op.get("new_text") or "").strip()
                if (act == "add" and target == "memory" and content2
                        and content2 not in working and _near_duplicate_of(content2, working) is not None):
                    continue  # near-dup of an existing entry — skip like an exact duplicate
                msg = self._apply_batch_op(working, act, content2,
